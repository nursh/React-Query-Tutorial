...
function IssueLabels({ labels, issueNumber }) {
  ...
  const setLabels = useMutation((labelId) => {
    ...
    onMutate: (labelId) => {
      ...
      const newLabels = oldLabels.includes(labelId)
        ? labels.filter((currentLabel) => currentLabel !== labelId)
        : [...oldLabels, labelId];
      queryClient.setQueryData(["issues", issueNumber], (data) => ({ ...data, labels: newLabels }));
      ...
      return function rollback() {
        queryClient.setQueryData(["issues", issueNumber], (data) => {
          ...
          return { ...data, label: rollbackLabels };
        })
      }
    },
        ...
        menuOpen && <div className="picker-menu labels">
          {
            labelsQuery.data?.map((label) => {
              ...
              return (
                <div key={label.id} className={selected ? "selected" : ""} onClick={() => setLabels.mutate(label.id)}>
